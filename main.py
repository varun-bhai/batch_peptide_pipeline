...
def main() -> None:
    ...
    for idx, (job_name, sequence) in enumerate(records, start=1):
        ...
        try:
            ...
            # ---------------------------------------------------------
            # THE DELIVERY TRUCK & CLEAN-UP CREW
            # ---------------------------------------------------------
            import shutil
            
            if os.path.exists(minimized_pdb):
                # 1. Determine if it Converged or Exploded by reading the log
                dest_folder = "converged"
                log_path = os.path.join(job_dir, "opt.log")
                if os.path.exists(log_path):
                    with open(log_path, "r") as f:
                        log_text = f.read()
                        if "Explosion detected" in log_text or "Energy is positive" in log_text or "Structure is disintegrating" in log_text:
                            dest_folder = "exploded"
                
                # 2. Count success/failure
                if dest_folder == "converged":
                    successes += 1
                    print(f"[INFO] Job '{job_name}' CONVERGED successfully.", flush=True)
                else:
                    failures += 1
                    print(f"[INFO] Job '{job_name}' EXPLODED. Saved for inspection.", flush=True)

                # 3. Drive the folder to Google Drive
                ...
                # If no PDB was generated, it was a hard crash (e.g., AlphaFold failed)
                failures += 1
                print(f"[ERROR] Job '{job_name}' failed completely. No final PDB generated.", flush=True)

            # 4. The Clean-Up Crew: Delete the local folder so there are NO duplicates
            if os.path.exists(job_dir):
                shutil.rmtree(job_dir)
                print(f"[INFO] Cleaned up local Colab workspace for '{job_name}'.", flush=True)
