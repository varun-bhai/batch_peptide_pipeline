...
def main():
    ...
    try:
        ...
        # --- NEW LOGIC: Box the product before leaving the factory ---
        if stats['exploded']:
        ...
        # Step 3: Reformat (This now runs whether it exploded or converged!)
        reformat_pdb(protonated_path, stats['ase_raw_path'], final_pdb)
        print(f"[MACE] Done. Final PDB saved to: {final_pdb}")

        # Now that the file is safely saved, we are allowed to trigger the exit
        if stats['exploded']:
            sys.exit(1)
